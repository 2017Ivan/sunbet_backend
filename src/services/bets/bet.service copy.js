...
/**
 * Normalize selection value - convert various formats to 1, X, 2
 */
const normalizeSelection = (selection) => {
  if (!selection) return null;
  
  // Convert to string and trim
  let value = String(selection).trim().toUpperCase();
  ...
  // Extract from patterns like "1X2 | Full Time - X"
  const patterns = [
    /[-\s]([1X2])$/,           // Matches "- X", "- 1", "- 2" at the end
    /\|\s*([1X2])$/,           // Matches "| X", "| 1", "| 2" at the end
    /FULL TIME - ([1X2])/,     // Matches "Full Time - X"
    /([1X2])\s*\|\s*/,         // Matches "1 |", "X |", "2 |"
  ];
  ...
  // Check common words
  if (value === 'HOME' || value === 'H' || value === '1') return '1';
  if (value === 'DRAW' || value === 'D' || value === 'X') return 'X';
  if (value === 'AWAY' || value === 'A' || value === '2') return '2';
  
  // Last resort - try to find any valid character
...
const validateSelections = (selections) => {
  ...
  console.log('Validating selections:', JSON.stringify(selections, null, 2));

  // Check each selection has required fields
  selections.forEach((selection, index) => {
    if (!selection.match || !selection.odds) {
    ...
    // Normalize the selection value
    let normalizedSelection = selection.selection;
    if (normalizedSelection) {
      normalizedSelection = normalizeSelection(normalizedSelection);
      // Update the selection with normalized value
      selection.selection = normalizedSelection;
    }
    
    console.log(`Selection ${index + 1} - Original: ${selection.selection}, Normalized: ${normalizedSelection}`);
    
    if (!normalizedSelection || !['1', 'X', '2'].includes(normalizedSelection)) {
      throw new ValidationError(`Selection ${index + 1} must be 1, X, or 2 (received: "${selection.selection}")`);
    }
  });
...
const placeBet = async (userId, selections, stake) => {
  ...
  console.log('Bet placed successfully:', betData.bookingCode);
  console.log('========================');
